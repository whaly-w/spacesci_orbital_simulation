...
class Satellite(Planet):
    ...
    def orbit_setup(self, planet, move= True, position = -1, direction= 1):
        '''
        position: -1, +1
        direction: -1, +1
        '''
        ...
        # Satellite setup
        self.orbit_position = (1.4 * self.target.radius * self.orbit_scale /planet.SCALE) * position
    ...
    def update_hohmann_transfer(self, start, min_distance= 3e9):
        error_compensate = 10 if self.target.distance_from_Sun / self.origin.distance_from_Sun> 3 else 0
